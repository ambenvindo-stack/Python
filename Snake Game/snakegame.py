...
background_color = pygame.Color(0,0,0)  # we willl fill background color as black
...
# to check the boundaries here we are not limiting boundaries like it can pass through screen and come from other si


def checkLimits(snake):
    if(snake.x > SCREEN_WIDTH):
        snake.x = SNAKE_SIZE
    if(snake.x < 0):  # this will be checked when some of snake is on other side and some opposite side
...
class Apple:
    ...
    def draw(self, screen):
        pygame. draw.rect(screen, self.color, (self.x, self.y, APPLE_SIZE),0)
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            ...
            # if we want to continue playing again
                ...
                return "yes" 
            # if we don't want to continue playing again
...
def main():
    ...
    # food
    max_apples = 1  # 1 when snake eats
